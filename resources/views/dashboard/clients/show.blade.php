...
@php
    ...
    $relationLabels = ['interested' => 'مهتم', 'viewed' => 'تمت المعاينة', 'reserved' => 'محجوز'];
    $typeMeta = [
    ...
    $editFields = ['name','phone','email','area_id','type_id','agent_id','source_id','desired_unit_type_id','social_status','nationality','household_size','workplace','in_person','visit_times','preferred_contact','property_address'];
@endphp

@section('content')
<div x-data="{ editOpen: {{ $errors->hasAny($editFields) ? 'true' : 'false' }}, propertySearch: '', selectedProperty: null }">
    @if (session('success'))
            ...
            <section class="rounded-card bg-white border border-gray-100 shadow-sm p-6">
                <div class="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if ($client->phone)<a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $client->phone) }}" target="_blank" rel="noopener" class="rounded-full bg-success/10 text-success hover:bg-success/20 text-sm px-4 py-2 font-medium">واتساب</a>@endif
                    </div>
                </div>

                <dl class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-5 mt-6 pt-5 border-t border-gray-100 text-sm">
                    @foreach ([
                        ['الهاتف', $client->phone], ['البريد', $client->email],
                        ['طريقة التواصل', $contactLabels[$client->preferred_contact] ?? null],
                        ['نوع الوحدة المطلوبة', $client->desiredUnitType?->name],
                        ['المنطقة المطلوبة', $client->area?->name], ['الجنسية', $client->nationality],
                        ['الحالة الاجتماعية', $socialLabels[$client->social_status] ?? null],
                        ['عدد الأفراد', $client->household_size], ['مكان العمل', $client->workplace],
                        ['حضوري', is_null($client->in_person) ? null : ($client->in_person ? 'نعم' : 'لا')],
                        ['أوقات الزيارة', $client->visit_times], ['مسؤول العقار', $client->agent?->name],
                        ['المصدر', $client->source?->name], ['سجّل البيانات', $client->recordedBy?->name],
                    ] as [$label, $value])
                        <div><dt class="text-gray-400 text-xs mb-1">{{ $label }}</dt><dd class="text-ink font-medium break-words">{{ filled($value) ? $value : '—' }}</dd></div>
                    @endforeach
                </dl>
                @if ($client->property_address)<div class="mt-5 pt-4 border-t border-gray-100"><p class="text-xs text-gray-400 mb-1">عنوان العقار المطلوب بالتفصيل</p><p class="text-sm text-gray-600">{{ $client->property_address }}</p></div>@endif
                ...
                @can('clients.edit')
                    <form method="POST" action="{{ route('dashboard.clients.interactions.store', $client) }}" class="rounded-field bg-gray-50 border border-gray-100 p-4 mb-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div><label class="block text-xs font-medium text-gray-600 mb-1">نوع التواصل</label><select name="type" class="w-full rounded-field border border-gray-200 bg-white px-3 py-2 text-sm">@foreach ($typeMeta as $key => $meta)<option value="{{ $key }}">{{ $meta['label'] }}</option>@endforeach</select></div>
    ...
    @can('clients.edit')
        <div x-show="editOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog">
            ...
            <div class="relative w-full max-w-6xl bg-white rounded-card shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form method="POST" action="{{ route('dashboard.clients.update', $client) }}">@csrf @method('PUT')<div class="p-6">@include('dashboard.clients._form', ['client' => $client])</div><div class="sticky bottom-0 flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50"><button type="button" @click="editOpen = false" class="rounded-full px-4 py-2.5 text-sm text-gray-600">إلغاء</button><button type="submit" class="rounded-full bg-primary-900 hover:bg-primary-800 text-white font-semibold px-5 py-2.5 text-sm">حفظ التعديلات</button></div></form>
            </div>
        </div>
    @endcan
</div>
