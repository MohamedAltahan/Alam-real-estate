...
@php
    ...
    $advancedKeys = ['agent_id', 'city_id', 'area_id', 'unit_type_id', 'nationality', 'social_status', 'preferred_contact', 'from', 'to', 'notes_q'];
...
<div x-data="{
        ...
        notesOpen: false, notesClient: null,
        targetOpen: false, targetClient: null,
        copyDone: false,
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-5">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="relative w-[260px] max-w-[55vw]">
                ...
                <input type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="بحث بالاسم أو الهاتف أو البريد..." autocomplete="off" form="clients-filters"
    ...
    <div x-show="filtersOpen" x-cloak x-collapse.duration.200ms
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
            ...
            <div>
                ...
                <select name="city_id" form="clients-filters" x-model="city" @change="area = ''; $refs.areaSelect.value = ''" class="{{ $filterSelect }}">
            ...
            <div>
                ...
                <select name="area_id" form="clients-filters" x-ref="areaSelect" x-model="area" class="{{ $filterSelect }}">
                    <option value="">كل المناطق</option>
                    <template x-for="a in areasFor()" :key="a.id"><option :value="a.id" x-text="a.name" :selected="String(a.id) === String(area)"></option></template>
                </select>
            </div>
            <div>
                ...
                <select name="unit_type_id" form="clients-filters" class="{{ $filterSelect }}">
                    ...
                    @foreach ($unitTypes as $type)<option value="{{ $type->id }}" @selected(($filters['unit_type_id'] ?? '') == $type->id)>{{ $type->name }}</option>@endforeach
                </select>
            </div>
            <div>
                <label class="{{ $filterLabel }}">الجنسية</label>
                <input name="nationality" form="clients-filters" list="filter-nationalities" value="{{ $filters['nationality'] ?? '' }}" placeholder="الكل" autocomplete="off" class="{{ $filterInput }}">
                <datalist id="filter-nationalities">@foreach ($nationalities as $n)<option value="{{ $n }}"></option>@endforeach</datalist>
            </div>
            <div>
                <label class="{{ $filterLabel }}">الحالة الاجتماعية</label>
                <select name="social_status" form="clients-filters" class="{{ $filterSelect }}">
                    <option value="">الكل</option>
                    @foreach (ClientFields::SOCIAL_STATUSES as $value => $text)<option value="{{ $value }}" @selected(($filters['social_status'] ?? '') === $value)>{{ $text }}</option>@endforeach
                </select>
            </div>
            <div>
                ...
                <select name="preferred_contact" form="clients-filters" class="{{ $filterSelect }}">
                    <option value="">الكل</option>
                    @foreach (ClientFields::CONTACT_METHODS as $value => $text)<option value="{{ $value }}" @selected(($filters['preferred_contact'] ?? '') === $value)>{{ $text }}</option>@endforeach
                </select>
            </div>
            <div>
                <label class="{{ $filterLabel }}">مسجّل من تاريخ</label>
                <input name="from" form="clients-filters" data-datepicker value="{{ $filters['from'] ?? '' }}" placeholder="من" class="{{ $filterInput }}">
            </div>
            <div>
                <label class="{{ $filterLabel }}">إلى تاريخ</label>
                <input name="to" form="clients-filters" data-datepicker value="{{ $filters['to'] ?? '' }}" placeholder="إلى" class="{{ $filterInput }}">
            </div>
            <div>
                <label class="{{ $filterLabel }}">بحث في الملاحظات</label>
                <input type="search" name="notes_q" form="clients-filters" data-min-length="{{ (int) config('clients.notes_min_length', 3) }}" value="{{ $filters['notes_q'] ?? '' }}" placeholder="3 أحرف على الأقل" autocomplete="off" class="{{ $filterInput }}">
            </div>
        </div>
        <div class="flex items-center justify-between gap-3 mt-3">
            <p class="text-[11px] text-gray-400">الفلاتر تُطبَّق تلقائيًا فور الاختيار.</p>
            <a href="{{ route('dashboard.clients.index') }}" data-filters-reset="clients-filters" class="text-sm text-gray-500 hover:text-danger">مسح كل الفلاتر</a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-start">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse ($clients as $c)
                        ...
                        <tr class="hover:bg-gray-50/50 transition cursor-pointer"
                            ...
                            <td class="px-4 py-3 max-w-[230px]">
                                <button type="button" @click.stop="targetClient = @js($targetPayload); targetOpen = true"
                                        class="w-full text-start rounded-xl px-2.5 py-2 hover:bg-primary-50 transition">
                                    @if ($firstViewing)
                                        <span class="flex items-center gap-2">
                                            <span class="font-semibold text-ink" dir="ltr">{{ $firstViewing->property?->reference_code ?: '—' }}</span>
                                            @if ($c->viewings->count() > 1)
                                                <span class="rounded-full bg-primary-100 text-primary-700 text-[11px] font-bold px-2 py-0.5">+{{ $c->viewings->count() - 1 }}</span>
                                            @endif
                                        </span>
                                        <span class="block text-xs text-gray-500 truncate">{{ $firstViewing->property?->title }}</span>
                                        <span class="block text-[11px] mt-0.5"><span class="rounded-full px-2 py-0.5 {{ ClientFields::outcomeTone($firstViewing->outcome) }}">{{ ClientFields::outcomeLabel($firstViewing->outcome) }}</span></span>
                                    @else
                                        <span class="block text-gray-400">لا يوجد عقار مستهدف</span>
                                        @if ($c->needs->isNotEmpty())
                                            <span class="block text-[11px] text-primary-600 mt-0.5 truncate">{{ $c->needs->first()->describe() }}</span>
                                        @endif
                                    @endif
                                </button>
                            </td>
                    ...
                    @empty
                        <tr><td colspan="8" class="px-4 py-16 text-center text-gray-400">لا يوجد عملاء مطابقون.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    {{-- ===== مودال إضافة عميل ===== --}}
    <div x-show="addOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" @keydown.escape.window="addOpen = false">
        <div class="absolute inset-0 bg-primary-950/50" @click="addOpen = false"></div>
        <div class="relative w-full max-w-6xl bg-white rounded-card shadow-2xl max-h-[90vh] overflow-y-auto" x-transition.opacity>
    ...
    <div x-show="targetOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" @keydown.escape.window="targetOpen = false">
        ...
        <div class="relative w-full max-w-2xl bg-white rounded-card shadow-2xl max-h-[85vh] overflow-y-auto">
            <div class="sticky top-0 bg-white z-10 flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div><h3 class="font-bold text-ink">العقارات المستهدفة</h3><p class="text-xs text-gray-400" x-text="targetClient?.name"></p></div>
                <button type="button" @click="targetOpen = false" class="grid place-items-center w-9 h-9 rounded-full hover:bg-gray-100 text-gray-500"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button>
            </div>
            <div class="p-6">
                <div class="rounded-2xl bg-gray-50 px-4 py-3 mb-5" x-show="(targetClient?.needs || []).length">
                    ...
                    <ul class="text-sm text-ink space-y-1"><template x-for="(need, i) in (targetClient?.needs || [])" :key="i"><li x-text="need"></li></template></ul>
                </div>
                <div class="space-y-3">
                    <template x-for="(item, index) in (targetClient?.viewings || [])" :key="index">
                        <article class="flex gap-3 rounded-2xl border border-gray-100 p-3">
                            <span class="w-16 h-14 rounded-xl bg-gray-100 overflow-hidden shrink-0"><img x-show="item.cover" :src="item.cover" class="w-full h-full object-cover" alt=""></span>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2 flex-wrap">
                                    <span class="font-semibold text-sm text-ink" dir="ltr" x-text="item.ref || '—'"></span>
                                    <span class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold" :class="item.tone" x-text="item.outcome"></span>
                                </div>
                                <p class="text-xs text-gray-500 truncate" x-text="[item.title, item.area].filter(Boolean).join(' · ')"></p>
                                <p class="text-xs text-gray-600 mt-1"><span class="text-gray-400">الموعد:</span> <span dir="ltr" x-text="item.at || '—'"></span> · <span x-text="item.in_person ? 'حضوري' : 'غير حضوري'"></span></p>
                                <p class="text-xs text-gray-600 mt-1 whitespace-pre-line" x-show="item.notes" x-text="item.notes"></p>
                            </div>
                        </article>
                    </template>
                    <p x-show="!(targetClient?.viewings || []).length" class="py-8 text-center text-sm text-gray-400">لا توجد معاينات مسجلة لهذا العميل حتى الآن.</p>
                </div>
            </div>
        </div>
    </div>
