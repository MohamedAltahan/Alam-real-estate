...
<div x-data="cityCrud()">
    ...
    @include('dashboard.areas._tabs', ['current' => 'cities'])
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($cities as $city)
                            @php
                                $nameAr = $city->getTranslation('name', 'ar', false);
                                $nameEn = $city->getTranslation('name', 'en', false);
                                $editData = [
                                    'id' => $city->id,
                                    'name_ar' => $nameAr,
                                    'name_en' => $nameEn,
                            ...
                            <tr class="hover:bg-gray-50/50">
                                ...
                                <td class="px-4 py-3 text-gray-600 tabular-nums">{{ $city->sort_order }}</td>
                                <td class="px-4 py-3">
                                    @if ($city->is_active)
    ...
    <x-modal name="city-form" maxWidth="lg">
    ...
    <x-modal name="city-delete" maxWidth="md">
        <div class="p-6 text-center">
            ...
            <form :action="delAction" method="POST" class="flex items-center justify-center gap-3">
                @csrf @method('DELETE')
                <button type="button" @click="$dispatch('close-modal', 'city-delete')" class="rounded-full px-4 py-2.5 text-sm text-gray-600 border border-gray-200 hover:bg-gray-100">إلغاء</button>
...
<script>
    function cityCrud() {
        return {
            ...
            init() {
                @if ($errors->any())
                    this.$nextTick(() => this.$dispatch('open-modal', 'city-form'));
                    this.action = '{{ route('dashboard.cities.store') }}';
                    this.form = { name_ar: @js(old('name.ar', '')), name_en: @js(old('name.en', '')), sort_order: @js(old('sort_order', $nextSortOrder)), is_active: true };
                @endif
            },
            startAdd() {
                this.mode = 'add';
                ...
                this.action = '{{ route('dashboard.cities.store') }}';
                this.$dispatch('open-modal', 'city-form');
            },
            startEdit(city) {
                this.mode = 'edit';
                this.form = {
                ...
                this.action = '{{ url('dashboard/cities') }}/' + city.id;
                this.$dispatch('open-modal', 'city-form');
            },
            startDelete(action, name) {
                this.delAction = action;
                this.delName = name;
                this.$dispatch('open-modal', 'city-delete');
            },
        };
    }
</script>
