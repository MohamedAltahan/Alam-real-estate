...
@section('content')
    {{-- ===================== الهيرو ===================== --}}
    <section class="relative isolate overflow-hidden text-white" x-data="{ i: 0, imgs: @js(array_values($hero['images'] ?? [])), ms: {{ (int) ($hero['rotate_seconds'] ?? 5) * 1000 }} }" x-init="if (imgs.length > 1) setInterval(() => i = (i + 1) % imgs.length, ms)">
        {{-- خلفية متحركة --}}
        ...
        <div class="relative max-w-5xl mx-auto px-4 sm:px-6 pt-44 pb-16 text-center">
            @if ($it($hero, 'badge'))
                <span
                    class="inline-flex items-center gap-2 rounded-full bg-white/10 border border-white/15 backdrop-blur-sm px-4 py-1.5 text-sm text-white/90 mb-5">
                    <span class="w-1.5 h-1.5 rounded-full bg-accent-500 animate-pulse"></span>{{ $it($hero, 'badge') }}
                </span>
            @endif
            <h1
                class="font-display text-[1.75rem] sm:text-[2rem] lg:text-[2.65rem] font-bold leading-[1.15] mb-3 text-balance drop-shadow-[0_2px_20px_rgba(0,0,0,0.35)]">
                {{ $it($hero, 'title') ?: $t('اعثر على العقار المثالي لك', 'Find your ideal property') }}</h1>
            @if ($it($hero, 'subtitle'))
                <p class="text-lg sm:text-xl lg:text-[1.6rem] font-semibold text-accent-400 mb-4">
                    {{ $it($hero, 'subtitle') }}</p>
            @endif
            {{-- الوصف يتمدّد لعرض الهيرو كاملاً على الشاشات الكبيرة حتى لا ينكسر في مساحة ضيّقة --}}
            @if ($it($hero, 'description'))
                <p class="text-base sm:text-lg text-white/75 max-w-2xl lg:max-w-none mx-auto mb-8 leading-relaxed">
            ...
            @php
                $mk = fn($col) => $col
                    ->map(fn($x) => ['v' => (string) $x->id, 'l' => (string) $x->name])
                    ->values()
                    ->all();
                $refItems = $searchReferences->map(fn($r) => ['v' => $r, 'l' => $r])->all();
                $bedItems = collect([1, 2, 3, 4, 5])
                    ->map(fn($n) => ['v' => (string) $n, 'l' => ($n === 5 ? '+5' : $n) . ' ' . $t('غرف', 'rooms')])
                    ->all();
                $priceItems = [
                            ...
                            @foreach ($searchCategories as $sc)
                                <button type="button" @click="cat = '{{ $sc->id }}'"
                                    ...
                                    :class="cat === '{{ $sc->id }}' ?
                                        ...
                                        'text-white/80 hover:text-white'">{{ $sc->name }}</button>
                            @endforeach
                        </div>
                    </div>
                @endif
                    ...
                    @foreach ([['properties', $t('عقار', 'Properties')], ['clients', $t('عميل', 'Clients')], ['areas', $t('منطقة', 'Areas')]] as [$k, $lbl])
                        @if (data_get($hero, "stats.$k"))
                            <div class="text-center">
                                <p class="text-xl sm:text-2xl font-bold text-white tabular-nums" dir="ltr">
                                    {{ data_get($hero, "stats.$k") }}</p>
                                <p class="text-[11px] text-white/60 mt-1">{{ $lbl }}</p>
                            </div>
                        @endif
                    @endforeach
                </div>
            ...
            <div x-show="imgs.length > 1" x-cloak class="flex justify-center mt-8">
                <div
                    class="inline-flex items-center gap-2 rounded-full bg-white/10 backdrop-blur-md border border-white/15 px-3 py-2">
                    <template x-for="(im, idx) in imgs" :key="idx">
    ...
    @if ($it($featured, 'title'))
        <section class="max-w-7xl mx-auto px-4 sm:px-6 mt-20 relative z-10">
            <div
                class="rounded-3xl navy-gradient text-white p-6 sm:p-8 flex flex-wrap items-center gap-5 justify-between overflow-hidden relative isolate">
                    ...
                    <span class="hidden sm:grid place-items-center w-16 h-16 shrink-0">
                        ...
                        @else
                            <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="1.4" class="text-accent-500">
                                <path d="M3 21h18M6 21V8l7-4v17M18 21V12l-5-2.5" stroke-linejoin="round" />
                                <path d="M9 9v.01M9 12v.01M9 15v.01M15.5 14v.01M15.5 17v.01" stroke-linecap="round"
                                    stroke-width="2" />
                            </svg>
                        @endif
                    </span>
                ...
                <a href="{{ route('site.properties') }}"
                    class="relative rounded-full gold-gradient hover:brightness-110 text-primary-900 font-semibold px-7 py-3 text-sm shadow-lg shadow-accent-500/20 transition">{{ $t('اكتشف العروض', 'Explore offers') }}</a>
            </div>
        </section>
    @endif
    ...
    @if ($videoProperties->count())
        <section class="max-w-7xl mx-auto px-4 sm:px-6 pt-10 pb-20" x-data="carousel">
                    ...
                    <a href="{{ $vp->video_url }}" target="_blank" rel="noopener"
                        ...
                        <div class="relative aspect-video bg-gray-100">
                            ...
                            <span class="absolute inset-0 bg-primary-950/15 grid place-items-center">
                                <span
                                    class="grid place-items-center w-12 h-12 rounded-full bg-white/25 backdrop-blur-sm border border-white/40 text-white group-hover:bg-accent-500 group-hover:text-primary-900 group-hover:border-accent-500 group-hover:scale-110 transition">
                        ...
                        <div class="p-4">
                            <span class="text-xs font-bold text-accent-600"
                                dir="ltr">{{ $vp->reference_code }}</span>
                            <div class="flex items-center justify-between gap-2 mt-1">
                                ...
                                <span class="flex items-center gap-1 text-xs text-gray-500 shrink-0">
                                    <span
                                        class="font-semibold text-ink">{{ $vp->rating ? number_format($vp->rating, 1) : '—' }}</span>
            ...
            <div class="text-center mt-9">
                <a href="{{ route('site.properties') }}"
                    class="inline-flex items-center gap-2 rounded-full border-2 border-primary-900 hover:navy-gradient hover:text-white text-primary-900 font-semibold px-7 py-3 text-sm transition">
    ...
    @if (!empty($why['items']))
        <section class="relative bg-gray-50 pt-10 pb-6 overflow-hidden" x-data="carousel">
            ...
            <div class="pointer-events-none absolute bottom-4 end-2 xl:end-6 hidden lg:block" aria-hidden="true">
                <svg width="150" height="245" viewBox="0 0 180 290" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <ellipse cx="95" cy="278" rx="85" ry="12" fill="#1D2026"
                        opacity="0.06" />
                    <rect x="14" y="96" width="34" height="182" rx="5" fill="#E9EAF0" />
                    <rect x="48" y="34" width="112" height="244" rx="6" fill="#DFE1E9" />
                    @for ($r = 0; $r < 6; $r++)
                        @for ($cIdx = 0; $cIdx < 3; $cIdx++)
                            <rect x="{{ 62 + $cIdx * 32 }}" y="{{ 56 + $r * 36 }}" width="18" height="23"
                                rx="3" fill="#FBD300" />
                        @endfor
                    @endfor
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 text-center">
                ...
                <div class="relative mb-8">
                    <span
                        class="inline-block rounded-full bg-accent-100 border border-accent-700 text-accent-700 px-3.5 py-1 text-xs font-bold mb-4">{{ $t('لماذا علم العقارية', 'Why us') }}</span>
                    <h2 class="text-2xl sm:text-4xl font-bold text-ink mb-2">
                        {{ $it($why, 'title') ?: $t('لماذا علم العقارية؟', 'Why Alam?') }}</h2>
                ...
                <div x-ref="track"
                    class="flex gap-5 overflow-x-auto snap-x scroll-smooth pb-1 text-start [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    @foreach ($why['items'] as $w)
                        <div class="snap-start shrink-0 w-[78%] sm:w-[46%] lg:w-[23.5%] rounded-2xl bg-white/80 p-6">
                            <div class="flex items-center justify-between gap-2 mb-4">
                                <span
                                    class="grid place-items-center w-11 h-11 rounded-full gold-gradient text-white shadow-sm shrink-0"><svg
                                        width="20" height="20" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2">
                                        <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                                    </svg></span>
                                <span class="text-3xl font-bold text-gray-300 tabular-nums"
                                    dir="ltr">{{ $w['number'] ?? '' }}</span>
                            </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6">
                ...
                <div x-ref="track"
                    ...
                    @foreach ($testimonials as $ts)
                        ...
                        <div
                            class="snap-start shrink-0 w-[78%] sm:w-[46%] lg:w-[23.5%] rounded-2xl bg-gray-50 border border-gray-100 p-5 flex flex-col">
                            <div class="text-sm mb-3">
                                <span class="text-accent-500">{{ str_repeat('★', $stars) }}</span><span
                                    class="text-gray-200">{{ str_repeat('★', max(0, 5 - $stars)) }}</span>
                            </div>
                            <p class="text-sm text-gray-600 leading-relaxed mb-5 flex-1">"{{ $ts->content }}"</p>
                            <div class="flex items-center gap-3 border-t border-gray-200 pt-4">
                                @if ($ts->avatar_url)
                                    <img src="{{ $ts->avatar_url }}" class="w-10 h-10 rounded-full object-cover shrink-0"
                                        alt="{{ $ts->name }}">
                                @else
                                    <span
                                        class="grid place-items-center w-10 h-10 rounded-full bg-primary-100 text-primary-700 font-bold text-sm shrink-0">{{ mb_substr($ts->name, 0, 1) }}</span>
                                @endif
                                <div class="min-w-0">
                                    <p class="font-semibold text-sm text-ink truncate">{{ $ts->name }}</p>
                                    <p class="text-xs text-gray-400 truncate">{{ $ts->title }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    {{-- ===================== ابدأ رحلتك ===================== --}}
    @if ($it($cta, 'title'))
        {{-- مسافة إضافية حول آخر قسم لفصله بصرياً عمّا قبله وعن الفوتر --}}
        <section class="max-w-7xl mx-auto px-4 sm:px-6 mt-4 mb-10 sm:mt-6 sm:mb-16">
            <div
                class="relative isolate rounded-[2.5rem] overflow-hidden bg-primary-900 text-white text-center p-10 sm:p-14">
                @if ($img($cta['image'] ?? null))
                    <img src="{{ $img($cta['image']) }}" class="absolute inset-0 w-full h-full object-cover"
                        alt="">
                ...
                <div class="relative">
                    @if ($it($cta, 'badge'))
                        <span
                            class="inline-block rounded-full bg-white/10 border border-white/20 backdrop-blur-sm text-accent-400 px-4 py-1.5 text-xs font-semibold mb-4">{{ $it($cta, 'badge') }}</span>
                    @endif
                    ...
                    <div class="flex items-center justify-center gap-3 flex-wrap">
                        <a href="{{ route('site.properties') }}"
                            class="rounded-full gold-gradient hover:brightness-110 text-primary-900 font-semibold px-7 py-3 text-sm shadow-lg shadow-accent-500/20 transition">{{ $t('ابدأ البحث الآن', 'Start searching') }}</a>
                        <a href="{{ route('site.contact') }}"
                            class="rounded-full bg-white/15 border border-white/25 backdrop-blur-sm hover:bg-white/25 text-white font-semibold px-7 py-3 text-sm transition">{{ $t('تواصل معنا', 'Contact us') }}</a>
                    </div>
                </div>
            </div>
        </section>
